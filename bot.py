...
try:
    ...
    # ===== ФОРМАТИРОВАНИЕ ARIAL 12 И БЕЛЫЙ ТЕКСТ ДЛЯ ВСЕХ ЯЧЕЕК =====
    ...
    if len(all_values) > 0 and len(all_values[0]) > 0:
        ...
        if rows > 0 and cols > 0:
            ...
            print(f"✅ Применено форматирование Arial 12 и белый текст ко всем {rows} строкам")

    # ===== ЗАПОЛНЕНИЕ ПРОПУЩЕННЫХ ДАТ В СТОЛБЦЕ "СРОК ПОГАШЕНИЯ" =====
    def fill_missing_expiration_dates():
        ...
        if date_col is None or exp_col is None:
            print("⚠️ Столбцы 'Дата нарушения' или 'Срок погашения' не найдены. Пропускаем заполнение.")
            return

        # Проходим по всем строкам, начиная со второй (0-based индекс 1)
        for row_idx in range(1, len(all_values)):
            row_data = all_values[row_idx]
            if len(row_data) <= max(date_col, exp_col):
                # Если строка короче, пропускаем
                continue
            date_val = row_data[date_col].strip()
            exp_val = row_data[exp_col].strip()
            if date_val and not exp_val:
                # Проверяем, что дата валидна (попробуем распарсить)
                try:
                    # Просто проверяем формат, но мы будем ставить формулу, даже если дата невалидна – Google сам обработает
                    pass
                except:
                    pass
                # Формируем формулу = [дата] + 21
                date_cell = f"{chr(65 + date_col)}{row_idx + 1}"  # +1 потому что row_idx 0-based, а A1-нотация 1-based
                formula = f"={date_cell}+21"
                # Обновляем ячейку срока погашения
                sheet.update(range_name=f"{chr(65 + exp_col)}{row_idx + 1}",
                             values=[[formula]],
                             value_input_option='USER_ENTERED')
                print(f"✅ Обновлён срок погашения для строки {row_idx+1}: {formula}")
    ...
    def setup_conditional_formatting():
        ...
        # Удаляем существующие правила условного форматирования (чтобы обновить)
        try:
            # Получаем текущий лист и удаляем все правила (если есть)
            spreadsheet = sheet.spreadsheet
            # Запрашиваем метаданные листа
            sheet_metadata = spreadsheet.get_sheet_metadata_by_title(sheet.title)
            # Удаляем все правила: отправляем пустой список
            # Но проще перезаписать новыми правилами, они заменят старые с таким же диапазоном
            pass
        except:
            pass
        ...
        try:
            ...
            print("✅ Условное форматирование для столбца 'Срок погашения' настроено (красный - активно, зелёный - истекло, белый текст).")
...
# ===================== ФУНКЦИЯ ПОИСКА АКТИВНЫХ НАРУШЕНИЙ =====================
...
# ===================== МОДАЛЬНЫЕ ОКНА ПОИСКА И ИЗМЕНЕНИЯ =====================
