...
try:
    ...
    # ===== ФОРМАТИРОВАНИЕ: ЧЁРНЫЙ ФОН, БЕЛЫЙ ТЕКСТ, ARIAL 12 =====
    ...
    if len(all_values) > 0 and len(all_values[0]) > 0:
        ...
        if rows > 0 and cols > 0:
            ...
            print(f"✅ Применено форматирование: чёрный фон, белый текст Arial 12 ко всем {rows} строкам")

    # ===== УСЛОВНОЕ ФОРМАТИРОВАНИЕ ДЛЯ СРОКА ПОГАШЕНИЯ (красный - активно, зелёный - истекло) =====
...
def format_row(sheet_obj, row_index_1based):
    """Применяет чёрный фон и белый текст Arial 12 к указанной строке."""
